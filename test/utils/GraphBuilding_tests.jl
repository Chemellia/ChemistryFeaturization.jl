...
@testset "GraphBuilding" begin
    adj, els =
        build_graph(abspath(@__DIR__, "../test_data", "mp-195.cif"), use_voronoi = true)
    wm_true = [0.0 1.0 1.0 1.0; 1.0 0.0 1.0 1.0; 1.0 1.0 0.0 1.0; 1.0 1.0 1.0 0.0]
    @test adj == wm_true
    @test els == ["Ho", "Pt", "Pt", "Pt"]

    adj, els =
        build_graph(abspath(@__DIR__, "../test_data", "mp-195.cif"); use_voronoi = false)
    @test adj == wm_true
    @test els == ["Ho", "Pt", "Pt", "Pt"]
    ...
    ) build_graph(abspath(@__DIR__, "../test_data", "methane.xyz"), use_voronoi = true)
