...
"""
function build_graphs_batch(input_folder::String, feature_names::Vector{Symbol}; nbins::Vector{<:Integer}=default_nbins*ones(Int64, size(feature_names,1)), logspaced=false, use_voronoi=false, radius=8.0, max_num_nbr=12, dist_decay_func=inverse_square, normalize=true, output_folder="", overwrite=true)
    atom_featurevecs, featurization = make_feature_vectors(build_featurization(feature_names; nbins=nbins, logspaced=logspaced))
    build_graphs_batch(input_folder, featurization; atom_featurevecs = atom_featurevecs, use_voronoi=use_voronoi, radius=radius, max_num_nbr=max_num_nbr, dist_decay_func=dist_decay_func, normalize=normalize, output_folder=output_folder, overwrite=overwrite)
end
