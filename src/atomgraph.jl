...
function normalized_laplacian(g::G) where G<:lg.AbstractGraph
    ...
    @assert !any(isnan.(lapl)) "NaN values in graph Laplacian! This is most likely due to atomic separations larger than the specified cutoff distance leading to block zeros in the adjacency matrix...try increasing the cutoff distance or inspecting your structure to ensure the file is correct."
    return lapl
end
