...
"Function for encoding onehot-style vectors based on values in a lookup table. Intended to be used as an `encode_f` for `AtomFeatureDescriptor` objects. See source code of convenience constructor for `AtomFeatureDescriptor` for more details."
...
"Function for decoding onehot-style vectors based on values in a lookup table. Intended to be used as an `decode_f` for `AtomFeatureDescriptor` objects. See source code of convenience constructor for `AtomFeatureDescriptor` for more details."
